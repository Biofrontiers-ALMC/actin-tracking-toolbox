...
stuckFibers_avgSpeed = mean([data.Tracks(stuckFibersID).AverageSpeed]);
notStuckFibers_avgSpeed = mean([data.Tracks(movingFibersID).AverageSpeed]);

data.maxStuckSpeed = max([data.Tracks(stuckFibersID).AverageSpeed])+.001; % The +.001 is to get around a greater than or equal comparision.
